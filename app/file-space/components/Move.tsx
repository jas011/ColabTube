...
function Comand({
  ...
  function renderFolderList(folder: string = "root") {
    setAddress(folder);
    //navigation
...
const Move = forwardRef<HTMLButtonElement, { file: any }>(({ file }, ref) => {
  ...
  const { folderMap, TrannslateFolder, fileMap, Route } = folderContext;
  let moveTo: any;
  let from: any;
  const handleMove = () => {
    if (file.type == "file") {
      ...
      from = folderMap.get(parentAddress);
      const newData = moveTo.addFolder(file);
      folderMap.set(newData.address, newData);
      TrannslateFolder.set(newData.key, newData.name);
      from.removeFolder(file.key);
      close.current?.click();
      ...
      <DialogContent className="overflow-hidden p-0 md:max-h-[500px] md:max-w-[700px] lg:max-w-[600px] h-[500px] w-[90%] outline-0 rounded">
        <SidebarProvider className="items-start">
          <main className="flex h-[480px] flex-1 flex-col overflow-hidden p-4">
            <h1
              className="px-3 scroll-m-20 text-xl font-bold tracking-tight lg:text-3xl"
              style={{
                width: "555px",
                height: "46px",
                whiteSpace: "nowrap",
...
// function moveFolder(from:any,to:any,folder:any,folderMap:any){
//   const iterator = (subfolder:any,path:string)=>{
//     path = path+subfolder.key;
//     subfolder.location = path;
//     if(subfolder.contents.folders.length>0){for(const folder of subfolder.contents.folders){
//       iterator(folder,path);
//     }}
//   }
//   const path = to.location;
//   iterator(folder,path);
//   console.log(folder);
